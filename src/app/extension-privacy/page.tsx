...
export const metadata: Metadata = {
  ...
  description: "How the Keepall Capture Chrome extension handles saved links and notes.",
};
    ...
    <main className="ui-scrollbar h-dvh overflow-y-auto bg-bg-shell p-2.5">
      <div className="library-panel min-h-full bg-bg-canvas px-5 py-5 sm:px-8 sm:py-7">
        ...
        <article className="mx-auto mt-12 w-full max-w-3xl pb-16 sm:mt-16">
          ...
          <p className="mt-3 text-sm text-text-secondary">Last updated September 23, 2026</p>
          ...
          <div className="mt-8 space-y-6 text-sm leading-7 text-text-secondary">
            <section>
              ...
              <p className="mt-2">When you click the toolbar button or use Alt/Option+K, Keepall Capture reads the current page URL and title. If you use the capture drawer, it also handles the title, note, Markdown choice, collection, and tags you enter or select. It does not read the full page text, your browsing history, cookies, or passwords.</p>
            </section>
            <section>
              <h2 className="text-lg font-semibold text-text-primary">Where your library lives</h2>
              <p className="mt-2">The extension passes the capture to the Keepall web app running in your browser. Keepall saves your library in that browser profile&apos;s local IndexedDB. You do not need a Keepall account, and the extension does not sync your library to a cloud account. The extension temporarily holds a pending capture in Chrome extension storage so a failed save can be retried. Pending captures expire after ten minutes and are removed at the next cleanup; confirmed captures are removed immediately. Your selected Keepall address remains in extension storage until you change it or remove the extension.</p>
            </section>
            <section>
              <h2 className="text-lg font-semibold text-text-primary">Network requests</h2>
              <p className="mt-2">The extension loads a Keepall page over HTTPS to write to the same local library without opening a visible tab. This request does not include your captured URL or note in its URL. When you view saved links in Keepall, the app may send a saved link URL to Keepall&apos;s preview service to fetch a title, description, or image from the linked website. The preview service and linked website may receive that URL. Notes, collections, and tags are not sent with preview requests.</p>
