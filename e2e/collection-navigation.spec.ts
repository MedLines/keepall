...
test.beforeEach(async ({ page }) => {
  ...
  await page.reload();
  await expect(page.getByRole("button", { name: "Design Inspiration", exact: true })).toBeVisible();
});
...
for (const edge of ["top", "centre", "bottom", "left"] as const) {
  test(`collection navigation works at the row ${edge}`, async ({ page }) => {
    const button = page.getByRole("button", { name: "Design Inspiration", exact: true });
...
test("collection actions do not navigate and keyboard navigation still works", async ({ page }) => {
  const design = page.getByRole("button", { name: "Design Inspiration", exact: true });
  await design.click();
